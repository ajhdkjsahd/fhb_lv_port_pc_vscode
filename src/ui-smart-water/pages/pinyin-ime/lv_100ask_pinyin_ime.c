 ...
 *      DEFINES
 *********************/
#define lv_memset_00(dst, len)  lv_memset((dst), 0, (len))
#define lv_textarea_del_char    lv_textarea_delete_char
...
static void lv_100ask_pinyin_ime_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    /* Create kb + cand_panel on the same parent as the pinyin_ime obj,
     * so input events reach them. (original used lv_scr_act() but we
     * create on lv_layer_top() which sits above scr_act.) */
    lv_obj_t * parent = lv_obj_get_parent(obj);
    if (parent == NULL) parent = lv_layer_top();
    pinyin_ime->kb = lv_keyboard_create(parent);
    ...
    pinyin_ime->cand_panel = lv_btnmatrix_create(parent);
    ...
    /* ===== DARK THEME: black-amber ===== */
    /* Container — click-through so kb+cand receive events */
    lv_obj_clear_flag(obj, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_set_style_bg_color(obj, lv_color_hex(0x0B1A24), 0);
    lv_obj_set_style_border_width(obj, 0, 0);

    /* ---- Candidate panel ---- */
    lv_obj_set_style_bg_color(pinyin_ime->cand_panel, lv_color_hex(0x0A1620), 0);
    lv_obj_set_style_border_width(pinyin_ime->cand_panel, 0, 0);
    /* Items: normal */
    lv_obj_set_style_text_color(pinyin_ime->cand_panel, lv_color_hex(0xD4A017), LV_PART_ITEMS);
    lv_obj_set_style_bg_opa(pinyin_ime->cand_panel, LV_OPA_0, LV_PART_ITEMS);
    /* Items: pressed */
    lv_obj_set_style_bg_opa(pinyin_ime->cand_panel, LV_OPA_COVER, LV_PART_ITEMS | LV_STATE_PRESSED);
    lv_obj_set_style_bg_color(pinyin_ime->cand_panel, lv_color_hex(0x1D3A52), LV_PART_ITEMS | LV_STATE_PRESSED);

    /* ---- Keyboard ---- */
    /* Main background */
    lv_obj_set_style_bg_color(pinyin_ime->kb, lv_color_hex(0x0B1A24), LV_PART_MAIN);
    lv_obj_set_style_border_width(pinyin_ime->kb, 0, LV_PART_MAIN);
    /* Keys: normal */
    lv_obj_set_style_bg_color(pinyin_ime->kb, lv_color_hex(0x15293A), LV_PART_ITEMS);
    lv_obj_set_style_text_color(pinyin_ime->kb, lv_color_hex(0xD4A017), LV_PART_ITEMS);
    lv_obj_set_style_radius(pinyin_ime->kb, 8, LV_PART_ITEMS);
    lv_obj_set_style_border_width(pinyin_ime->kb, 0, LV_PART_ITEMS);
    lv_obj_set_style_shadow_width(pinyin_ime->kb, 2, LV_PART_ITEMS);
    lv_obj_set_style_shadow_color(pinyin_ime->kb, lv_color_hex(0x0C1A26), LV_PART_ITEMS);
    lv_obj_set_style_shadow_ofs_y(pinyin_ime->kb, 2, LV_PART_ITEMS);
    /* Keys: pressed */
    lv_obj_set_style_bg_color(pinyin_ime->kb, lv_color_hex(0x1D3A52), LV_PART_ITEMS | LV_STATE_PRESSED);
    lv_obj_set_style_text_color(pinyin_ime->kb, lv_color_hex(0xE8C547), LV_PART_ITEMS | LV_STATE_PRESSED);
    /* Keys: checked (e.g. shift lock) */
    lv_obj_set_style_bg_color(pinyin_ime->kb, lv_color_hex(0x1D3A52), LV_PART_ITEMS | LV_STATE_CHECKED);
    lv_obj_set_style_text_color(pinyin_ime->kb, lv_color_hex(0x00D4AA), LV_PART_ITEMS | LV_STATE_CHECKED);
...
static void pinyin_input_proc(lv_obj_t * obj)
{
    ...
    /* Refresh btnmatrix to show updated pinyin_str content */
    lv_btnmatrix_set_map(pinyin_ime->cand_panel,
                         (const char **)btnm_def_pinyin_sel_map);
}
...
static void pinyin_page_proc(lv_obj_t* obj, uint16_t dir)
{
    ...
    if(dir == 0)
    {
        if(pinyin_ime->py_page == 0) return; /* Already on first page */
        pinyin_ime->py_page--;
    }
    else
    {
    ...
    /* Refresh btnmatrix after updating pinyin_str content */
    lv_btnmatrix_set_map(pinyin_ime->cand_panel,
                         (const char **)btnm_def_pinyin_sel_map);
}
