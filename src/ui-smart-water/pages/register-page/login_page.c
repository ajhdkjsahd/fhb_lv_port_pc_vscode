...
static void on_login_success_timer(lv_timer_t * tmr);
...
lv_obj_t * login_page_create(login_verify_cb_t       verify_cb,
                             ...
                             post_success_cb_t         post_success_cb)
{
    /* Allocate page context */
    login_page_ctx_t * ctx = lv_malloc(sizeof(login_page_ctx_t));
    if(ctx == NULL) return NULL;
    memset(ctx, 0, sizeof(login_page_ctx_t));
    ctx->verify_cb       = verify_cb;
    ctx->nav_cb          = nav_cb;
    ctx->post_success_cb = post_success_cb;
    ctx->success_timer   = NULL;
    ...
    /* Clean up context when screen is deleted */
    lv_obj_add_event_cb(screen, on_page_delete, LV_EVENT_DELETE, ctx);

    /* Background click dismisses keyboard */
    ...
    lv_textarea_set_placeholder_text(ctx->user_ta, "请输入账号");
    ...
    lv_textarea_set_placeholder_text(ctx->pass_ta, "请输入密码");
    ...
    /* Login button */
    lv_obj_t * login_btn = lv_button_create(btn_row);
    lv_obj_set_flex_grow(login_btn, 1);
    lv_obj_set_height(login_btn, 44);
    lv_obj_set_style_radius(login_btn, 12, 0);
    lv_obj_set_style_bg_color(login_btn, lv_color_hex(0x00D4AA), 0);
    lv_obj_set_style_bg_grad_color(login_btn, lv_color_hex(0x0288D1), 0);
    lv_obj_set_style_bg_grad_dir(login_btn, LV_GRAD_DIR_VER, 0);
    lv_obj_set_style_border_width(login_btn, 0, 0);
    lv_obj_set_style_shadow_width(login_btn, 12, 0);
    lv_obj_set_style_shadow_color(login_btn, lv_color_hex(0x00D4AA), 0);
    lv_obj_set_style_shadow_opa(login_btn, LV_OPA_30, 0);
    lv_obj_set_style_shadow_ofs_y(login_btn, 4, 0);
    NO_SCROLL(login_btn);
    lv_obj_add_event_cb(login_btn, on_login_click, LV_EVENT_CLICKED, ctx);

    lv_obj_t * login_lbl = lv_label_create(login_btn);
    lv_label_set_text(login_lbl, "登 录");
    lv_obj_set_style_text_font(login_lbl, app_font_kaiti_18(), 0);
    lv_obj_set_style_text_color(login_lbl, lv_color_white(), 0);
    ...
    lv_obj_set_style_bg_color(reg_btn, lv_color_hex(0x0A1620), 0);
    lv_obj_set_style_border_width(reg_btn, 1, 0);
    lv_obj_set_style_border_color(reg_btn, lv_color_hex(0x1C2E36), 0);
    NO_SCROLL(reg_btn);
    ...
    lv_obj_set_style_text_color(reg_lbl, lv_color_hex(0x9AB8B0), 0);

    /* ---- Hint ---- */
    lv_obj_t * hint = lv_label_create(form);
    lv_label_set_text(hint, "点击输入框唤出键盘 · 点击空白收起");
...
static void on_bg_click(lv_event_t * e)
{
    ...
    /* Only dismiss keyboard when clicking directly on the screen background */
    if(target == screen) {
        app_keyboard_hide();
    }
}
...
static void on_ta_click(lv_event_t * e)
{
    ...
    /* Determine the screen this textarea belongs to */
    lv_obj_t * parent = lv_obj_get_parent(ta);
    while(parent) {
        if(lv_obj_get_parent(parent) == NULL) break;
        parent = lv_obj_get_parent(parent);
    }

    lv_obj_t * screen = parent; /* root (screen) */
    app_keyboard_show(screen, ta, label);
}

static void on_login_click(lv_event_t * e)
{
    login_page_ctx_t * ctx = lv_event_get_user_data(e);
    ...
    /* Call user verification callback */
    if(ctx->verify_cb && ctx->verify_cb(username, password)) {
        app_popup_show(ctx->screen, "登录成功", APP_POPUP_SUCCESS);
        /* Schedule post-success navigation after popup shows */
        ctx->success_timer = lv_timer_create(on_login_success_timer, 500, ctx);
        lv_timer_set_repeat_count(ctx->success_timer, 1);
    }
    else {
        app_popup_show(ctx->screen, "账号或密码错误", APP_POPUP_ERROR);
    }
}
...
{
    login_page_ctx_t * ctx = lv_event_get_user_data(e);
    if(ctx->success_timer) {
        lv_timer_delete(ctx->success_timer);
        ctx->success_timer = NULL;
    }
    lv_free(ctx);
}
...
static void on_login_success_timer(lv_timer_t * tmr)
{
    login_page_ctx_t * ctx = lv_timer_get_user_data(tmr);
    ctx->success_timer = NULL;

    /* Dismiss popup */
    app_popup_dismiss();

    /* Call post-success callback for navigation */
    if(ctx->post_success_cb) {
        ctx->post_success_cb();
    }
